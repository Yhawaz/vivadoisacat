        ...
        for (i=0; i<3; i=i+1)begin
            ...
            always @(posedge clk)begin
                if (rst)begin
                    ...
                    shifted_pixel[i] <= $signed(fir_pixel[i]) >>> scaler;
                end
            end
            always @(posedge clk)begin
                if (rst)begin
                    ...
                    if (ob)begin
                      ...
                      //clipped_pixel[i] <= shifted_pixel[i];
                         clipped_pixel[i]<= ($signed(shifted_pixel[i])>32'sd127) ? 32'sd127:($signed(shifted_pixel[i])<-32'sd128)? -32'sd128:shifted_pixel[i];
                    end else begin
                        //6S965 Student: CHANGED
                         clipped_pixel[i]<= ($signed(shifted_pixel[i])>32'sd255) ? 32'sd255:($signed(shifted_pixel[i])<32'sd0)? 0:shifted_pixel[i];
                    end
                end
            end
        end
